...
## -----------------------------------------------------------------------------
## make_gif_videos(): animate selected variables for each resolution.
##
## Args:
##   resolutions   resolution rows to animate (default: ALL, F00..F04). F00 reads
##                 native stacks; F01..F04 read their regridded stacks.
##   gif_prefixes  which <var>_<depth> variables to animate
##   start_year/num_years   year window to animate
make_gif_videos <- function(resolutions   = resolution_set(),
  ...
  for (r in seq_len(nrow(resolutions))) {
    ...
    dir_out   <- file.path(out_root, row$res_dir, "GIFs")
...
if (!orchestrated()) make_gif_videos()
