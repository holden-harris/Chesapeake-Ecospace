...
pak::pkg_install("RAquaMaps/aquamapsdata")
...
sp_list         <- read.csv("./data-inputs/species-info/species-list.csv") ## 'sp_list' represents Ecospace functional groups
sp_list$Genus   <- word(sp_list$Sciname, 1)
sp_list$Species <- word(sp_list$Sciname, 2)
nrow(sp_list) ## Check number of functional groups

## Use only rows with genus and species
sp_list <- sp_list %>% filter(!is.na(Species)); nrow(sp_list)
...
write.csv(fg_pref, "./data-inputs/env-preference-functions/fg-env-preference-parameters.csv", row.names = FALSE)
